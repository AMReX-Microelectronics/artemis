...
​
            ...
            Box const& tbx = mfi.tilebox(Mxfield[0]->ixType().toIntVect()); /* just define which grid type */
            Box const& tby = mfi.tilebox(Mxfield[1]->ixType().toIntVect());
            Box const& tbz = mfi.tilebox(Mxfield[2]->ixType().toIntVect());
​
...
​
              ...
              Mx(i, j, k, 0) += dt * cons1 * ( Mx(i, j, k, 1) * Bz_xtemp(i, j, k) - Mx(i, j, k, 2) * By_xtemp(i, j, k))
                + dt * cons2 * ( Mx(i, j, k, 1) * (Mx(i, j, k, 0) * By_xtemp(i, j, k) - Mx(i, j, k, 1) * Bx(i, j, k))
                - Mx(i, j, k, 2) * ( Mx(i, j, k, 2) * Bx(i, j, k) - Mx(i, j, k, 0) * Bz_xtemp(i, j, k)));
​
              // y component on x-faces of grid
              Mx(i, j, k, 1) += dt * cons1 * ( Mx(i, j, k, 2) * Bx(i, j, k) - Mx(i, j, k, 0) * Bz_xtemp(i, j, k))
                + dt * cons2 * ( Mx(i, j, k, 2) * (Mx(i, j, k, 1) * Bz_xtemp(i, j, k) - Mx(i, j, k, 2) * By_xtemp(i, j, k))
                - Mx(i, j, k, 0) * ( Mx(i, j, k, 0) * By_temp(i, j, k) - Mx(i, j, k, 1) * Bx(i, j, k)));
​
              // z component on x-faces of grid
              Mx(i, j, k, 2) += dt * cons1 * ( Mx(i, j, k, 0) * By_xtemp(i, j, k) - Mx(i, j, k, 1) * Bx(i, j, k))
                + dt * cons2 * ( Mx(i, j, k, 0) * ( Mx(i, j, k, 2) * Bx(i, j, k) - Mx(i, j, k, 0) * Bz_xtemp(i, j, k))
                - Mx(i, j, k, 1) * ( Mx(i, j, k, 1) * Bz_xtemp(i, j, k) - Mx(i, j, k, 2) * By_xtemp(i, j, k)));
            },
​
            [=] AMREX_GPU_DEVICE (int i, int j, int k){
              ...
              My(i, j, k, 0) += dt * cons1 * ( My(i, j, k, 1) * Bz_ytemp(i, j, k) - My(i, j, k, 2) * By(i, j, k))
                + dt * cons2 * ( My(i, j, k, 1) * (My(i, j, k, 0) * By(i, j, k) - My(i, j, k, 1) * Bx(i, j, k))
                - My(i, j, k, 2) * ( My(i, j, k, 2) * Bx_ytemp(i, j, k) - My(i, j, k, 0) * Bz_ytemp(i, j, k)));
​
              // y component on y-faces of grid
              My(i, j, k, 1) += dt * cons1 * ( My(i, j, k, 2) * Bx_ytemp(i, j, k) - My(i, j, k, 0) * Bz_ytemp(i, j, k))
                + dt * cons2 * ( My(i, j, k, 2) * (My(i, j, k, 1) * Bz_ytemp(i, j, k) - My(i, j, k, 2) * By(i, j, k))
                - My(i, j, k, 0) * ( My(i, j, k, 0) * By_temp(i, j, k) - My(i, j, k, 1) * Bx_ytemp(i, j, k)));
​
              // z component on y-faces of grid
              My(i, j, k, 2) += dt * cons1 * ( My(i, j, k, 0) * By(i, j, k) - My(i, j, k, 1) * Bx_ytemp(i, j, k))
                + dt * cons2 * ( My(i, j, k, 0) * ( My(i, j, k, 2) * Bx_ytemp(i, j, k) - My(i, j, k, 0) * Bz_ytemp(i, j, k))
                - My(i, j, k, 1) * ( My(i, j, k, 1) * Bz_ytemp(i, j, k) - My(i, j, k, 2) * By(i, j, k)));   
            },
​
            [=] AMREX_GPU_DEVICE (int i, int j, int k){
              ...
              Mz(i, j, k, 0) += dt * cons1 * ( Mz(i, j, k, 1) * Bz(i, j, k) - Mz(i, j, k, 2) * By_ztemp(i, j, k))
                + dt * cons2 * ( Mz(i, j, k, 1) * (Mz(i, j, k, 0) * By_ztemp(i, j, k) - Mz(i, j, k, 1) * Bx(i, j, k))
                - Mz(i, j, k, 2) * ( Mz(i, j, k, 2) * Bx_ztemp(i, j, k) - Mz(i, j, k, 0) * Bz(i, j, k)));
​
              // y component on z-faces of grid
              Mz(i, j, k, 1) += dt * cons1 * ( Mz(i, j, k, 2) * Bx_ztemp(i, j, k) - Mz(i, j, k, 0) * Bz(i, j, k))
                + dt * cons2 * ( Mz(i, j, k, 2) * (Mz(i, j, k, 1) * Bz(i, j, k) - Mz(i, j, k, 2) * By_ztemp(i, j, k))
                - Mz(i, j, k, 0) * ( Mz(i, j, k, 0) * By_ztemp(i, j, k) - Mz(i, j, k, 1) * Bx_ztemp(i, j, k)));
​
              // z component on z-faces of grid
              Mz(i, j, k, 2) += dt * cons1 * ( Mz(i, j, k, 0) * By_ztemp(i, j, k) - Mz(i, j, k, 1) * Bx_ztemp(i, j, k))
                + dt * cons2 * ( Mz(i, j, k, 0) * ( Mz(i, j, k, 2) * Bx_ztemp(i, j, k) - Mz(i, j, k, 0) * Bz(i, j, k))
                - Mz(i, j, k, 1) * ( Mz(i, j, k, 1) * Bz(i, j, k) - My(i, j, k, 2) * By_ztemp(i, j, k))); 
            }
            );
        }
    }