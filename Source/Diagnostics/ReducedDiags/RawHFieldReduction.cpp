...
//constructor
RawHFieldReduction::RawHFieldReduction (std::string rd_name)
: ReducedDiags{rd_name}
{
    ...
    AMREX_ALWAYS_ASSERT_WITH_MESSAGE(false,
        "RawHFieldReduction reduced diagnostics does not work for RZ coordinate.");
#endif
    ...
    AMREX_ALWAYS_ASSERT_WITH_MESSAGE(nLevel == 0,
        "RawHFieldReduction reduced diagnostics does not work with mesh refinement.");
...
void RawHFieldReduction::ComputeDiags (int step)
{
    ...
    {
        ComputeRawHFieldReduction<amrex::ReduceOpMax>();
    }
    else if (m_reduction_type == ReductionType::Minimum)
    {
        ComputeRawHFieldReduction<amrex::ReduceOpMin>();
    }
    else if (m_reduction_type == ReductionType::Sum)
    {
        ComputeRawHFieldReduction<amrex::ReduceOpSum>();
    }
}
